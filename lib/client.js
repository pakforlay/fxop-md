...
const connect = async () => {
	...
	conn.ev.on("connection.update", clientConnection(conn));
	...
	conn.ev.on("messages.upsert", clientMsgs(conn));
...
const clientConnection = conn => async s => {
...
const clientMsgs = conn => async m => {
	...
	if (config.ANTILINK && msg.from.endsWith("@g.us")) {
		await clientAntilinks(msg, conn);
	}
...
const clientAntilinks = async (msg, conn) => {
